...
proberecord *newproberecord(void)
{
	...
	record = (proberecord*)xrealloc(NULL, sizeof(proberecord));
	record->state = (char*)xrealloc(NULL, TEXTSIZE);
	record->string = (char*)xrealloc(NULL, TEXTSIZE);
	return record;
}
...
void probe(proberecord *record, int ttl, int q)
{
	...
	if (o_pktlen && payload == NULL)
	{
		debug("Initializing payload of %d bytes\n", o_pktlen);
		payload = (u_char*)xrealloc(payload, o_pktlen);
